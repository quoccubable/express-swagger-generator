		...
		fn: {
			opsFilter: function (taggedOps, phrase) {
				...
				for (tagObj in normalTaggedOps) {
					if (
						normalTaggedOps[tagObj].tagDetails
						&& normalTaggedOps[tagObj].tagDetails.name.toLowerCase().indexOf(phrase) === -1
						&& normalTaggedOps[tagObj].tagDetails.description.toLowerCase().indexOf(phrase) === -1
					) {
...
function generateSpecAndMount(app) {
    ...
    return function (options) {
		...
		const swaggerOpts = swaggerObject.opts;

		if ( swaggerOpts && swaggerOpts.swaggerOptions && swaggerOpts.swaggerOptions.filter ) {
			if ( !swaggerOpts.swaggerOptions.plugins ) swaggerOpts.swaggerOptions.plugins = [];

			swaggerOpts.swaggerOptions.plugins.push( advancedFilterPlugin )
		}
        ...
        app.use(url, swaggerUi.serve, swaggerUi.setup(swaggerObject, swaggerOpts));
